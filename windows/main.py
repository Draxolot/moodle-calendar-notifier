...
def pushNotif(title, msg, link = True, duration = "short"):
    notif = Notification(app_id = os.getenv("APP_ID"),
                     title = title,
                     msg = msg,
                     duration = duration)
    notif.icon = os.path.abspath(os.getenv("ICON"))
    if link:
        notif.add_actions(label="Ouvrir", launch=os.getenv("CALENDAR_LINK"))
    notif.show()
...
def main():
    ...
    if calendar is None:
        pushNotif("Error", "Cannot access calendar")
        return
